...
Main deployment script for the Streamlit application
"""
...
# Try importing core dependencies with helpful error messages
try:
    # These imports are critical and must succeed
    import streamlit as st
    import pandas as pd
    import numpy as np
    import matplotlib.pyplot as plt
    ...
    if 'debug' in st.query_params and st.query_params['debug'] == 'true':
        st.success("Successfully imported core dependencies")
except ImportError as e:
    st.error(f"Critical error importing core dependencies: {e}")
    st.info("Please make sure the following packages are installed: streamlit, pandas, numpy, matplotlib, seaborn")
    st.info("Run: pip install -r requirements.txt")
    st.stop()

# Import custom modules - ensure they're in the correct path
def import_custom_modules():
    """Import custom modules with proper error handling"""
    # Ensure the current directory and app directory are in path
    sys.path.append(os.path.dirname(os.path.abspath(__file__)))
    
    # For deployment, we may have different directory structures
    possible_module_paths = [
        '.',  # Current directory
        'app',  # app subdirectory
        os.path.join(os.path.dirname(os.path.abspath(__file__)), 'app')  # Absolute path to app
    ]
    ...
    try:
        ...
        from data_processing import process_las_file, evaluate_clusters, extract_well_metadata, cluster_data, identify_continuous_segments, generate_node_connections, generate_edge_features, assign_hydrocarbon_potential
        ...
        # Store the imported functions in the modules dictionary
        modules.update({
            ...
            'create_3d_graph_visualization': create_3d_graph_visualization,
            'create_comparison_heatmap': create_comparison_heatmap,
            'create_depth_profile_visualization': create_depth_profile_visualization,
            'create_download_link': create_download_link,
            'create_figure_download_link': create_figure_download_link,
            'save_uploaded_file': save_uploaded_file,
            'create_project_folder': create_project_folder,
            'save_project_metadata': save_project_metadata,
            'load_project_metadata': load_project_metadata,
            'format_time': format_time,
            'display_progress': display_progress,
            'create_export_zip': create_export_zip,
            'create_download_button_for_file': create_download_button_for_file,
            'get_hydrocarbon_potential_color_scale': get_hydrocarbon_potential_color_scale,
            'create_summary_report': create_summary_report,
            'show_colorbar_legend': show_colorbar_legend
        })
        
        return modules, None
    except ImportError as e:
        error_messages.append(f"Error importing modules: {str(e)}")
        # Try an alternative approach - dynamically import modules
        try:
            for module_name in ['data_processing', 'gcn_model', 'network_visualization', 'utils']:
                for path in possible_module_paths:
                    try:
                        if path not in sys.path:
                            sys.path.append(path)
                        ...
                        break  # Stop if successfully imported
                    except ImportError:
                        continue
            ...
            if modules:
                ...
                return None, "Could not import required modules from any path"
        except Exception as e:
            return None, f"Modules could not be imported: {str(e)}\nPaths tried: {possible_module_paths}"
...
def data_upload_page(modules):
    ...
    if uploaded_file is not None:
        ...
        tmp_filepath = modules['save_uploaded_file'](uploaded_file)
        process_data = True
        
        st.markdown('<div class="highlight">', unsafe_allow_html=True)
        st.markdown(f"<p>File <b>{uploaded_file.name}</b> successfully uploaded! Ready to process data.</p>", unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
    elif use_sample:
        # Use sample data
        sample_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'app', 'sample-las.txt')
        if not os.path.exists(sample_path):
            # Try alternate paths for the sample data
            alternate_paths = [
                'sample-las.txt',
                os.path.join('app', 'sample-las.txt'),
                os.path.join(os.path.dirname(os.path.abspath(__file__)), 'sample-las.txt')
            ]
        ...
        if os.path.exists(sample_path):
            tmp_filepath = sample_path
            process_data = True
            
            st.markdown('<div class="highlight">', unsafe_allow_html=True)
            st.markdown("<p>Using sample data for demonstration. Ready to process data.</p>", unsafe_allow_html=True)
            st.markdown('</div>', unsafe_allow_html=True)
        else:
            st.error("Sample data file not found. Please upload your own LAS file.")
    
    # Process button
    if process_data and st.button("Process Data"):
        # Process the LAS file
        with st.spinner('Processing LAS file...'):
            result, message = modules['process_las_file'](tmp_filepath)
        ...
        if result is None:
            ...
            metadata = modules['extract_well_metadata'](result['las'])
            ...
            with st.expander("Find Optimal Number of Clusters", expanded=False):
                ...
                if st.button("Evaluate Clusters"):
                    with st.spinner("Evaluating clusters..."):
                        scores, optimal_clusters = modules['evaluate_clusters'](result['features'])
...
def graph_dataset_page(modules):
    """Render the graph dataset creation page"""
    ...
    if st.button("Generate Graph Dataset"):
        with st.spinner("Generating graph dataset..."):
            ...
            # Perform clustering
            modules['display_progress'](progress_bar, 10, "Performing K-means clustering...")
            clustered_df, kmeans_model = modules['cluster_data'](st.session_state['result']['features'], n_clusters)
            
            # Identify continuous segments
            modules['display_progress'](progress_bar, 30, "Identifying continuous facies segments...")
            segments = modules['identify_continuous_segments'](clustered_df['DEPTH'].values, clustered_df['Facies_pred'].values)
            
            # Generate node connections
            modules['display_progress'](progress_bar, 50, "Generating node connections...")
            node_data, split_indices = modules['generate_node_connections'](clustered_df, segments, train_test_split)
            
            # Generate edge features
            modules['display_progress'](progress_bar, 70, "Generating edge features...")
            edge_data = modules['generate_edge_features'](clustered_df)
            
            # Assign hydrocarbon potential labels
            modules['display_progress'](progress_bar, 90, "Assigning hydrocarbon potential labels...")
            labeled_data = modules['assign_hydrocarbon_potential'](edge_data)
            
            # Complete progress
            modules['display_progress'](progress_bar, 100, "Graph dataset creation complete!")
    ...
    if all(k in st.session_state['result'] for k in ['clustered_data', 'node_data', 'edge_data']):
        ...
        facies_fig = modules['create_facies_visualization'](
        ...
        pet_fig = modules['create_pet_distribution_plot'](st.session_state['result']['edge_data'])
        st.pyplot(pet_fig)
...
def graph_visualization_page(modules):
    ...
    if visualization_type == "2D Network Graph":
        ...
        with st.spinner("Generating 2D graph visualization..."):
            ...
            try:
                graph_fig = modules['create_network_graph'](
                ...
                st.error(f"Error generating 2D network graph: {str(e)}")
        ...
        with st.spinner("Generating 3D graph visualization..."):
            ...
            try:
                graph_3d_fig = modules['create_3d_graph_visualization'](
                    st.session_state['result']['node_data'], 
                    st.session_state['result']['edge_data'], 
                    st.session_state['result']['segments'],
                    sample_size=sample_size
                ...
                st.error(f"Error generating 3D network graph: {str(e)}")
    
    elif visualization_type == "Feature Importance":
        # Create feature importance visualization
        with st.spinner("Generating feature importance visualization..."):
            try:
                feature_importance_fig = modules['create_feature_importance_plot'](
                    st.session_state['result']['edge_data'],
        ...
        with st.spinner("Generating depth profile visualization..."):
            try:
                ...
                color_map = modules['get_hydrocarbon_potential_color_scale']()
                ...
                st.error(f"Error generating depth profile visualization: {str(e)}")
    ...
    with st.spinner("Generating facies vs hydrocarbon potential visualization..."):
        try:
            ...
            st.error(f"Error generating facies vs hydrocarbon potential visualization: {str(e)}")
...
def model_prediction_page(modules):
    ...
    if st.button("Run Prediction"):
        with st.spinner("Running GCN model for hydrocarbon potential prediction..."):
            ...
            if use_real_model:
                ...
                predicted_df = modules['simulate_gcn_prediction'](st.session_state['result']['edge_data'])
    ...
    if 'predicted_df' in st.session_state.get('result', {}):
        ...
        try:
            comparison_fig = modules['create_comparison_heatmap'](st.session_state['result']['predicted_df'])
            st.pyplot(comparison_fig)
        except Exception as e:
            st.error(f"Error creating comparison heatmap: {str(e)}")
        ...
        try:
            depth_profile_fig = modules['create_depth_profile_visualization'](st.session_state['result']['predicted_df'])
            st.plotly_chart(depth_profile_fig, use_container_width=True)
        except Exception as e:
            st.error(f"Error creating depth profile visualization: {str(e)}")
...
def results_interpretation_page(modules):
    """Render the results interpretation page"""
    ...
    with tab1:
        ...
        try:
            ...
            st.error(f"Error creating potential distribution visualization: {str(e)}")
    ...
    with tab2:
        ...
        try:
            ...
            st.error(f"Error creating depth analysis visualization: {str(e)}")
    ...
    with tab3:
        ...
        try:
            ...
            st.error(f"Error creating well log visualization with predictions: {str(e)}")
    ...
    if 'project_folder' not in st.session_state:
        project_folder = modules['create_project_folder']("vhydro_project")
    ...
    with col1:
        if st.button("Export Prediction Results (CSV)"):
            try:
                ...
                st.markdown(modules['create_download_link'](
                    st.session_state['result']['predicted_df'],
                    "hydrocarbon_prediction_results.csv",
                    "Download CSV File"
                ), unsafe_allow_html=True)
            except Exception as e:
                st.error(f"Error exporting prediction results: {str(e)}")
...
def main():
    ...
    modules, error = import_custom_modules()
    if error:
        st.error(f"Module import error: {error}")
        st.stop()
